...
void createContext() {
	...
	// We need a texture to store the depth image //h gl soil ta ekane aytomata alla den exoyme etoimh eikona twra
...
void depth_pass(mat4 viewMatrix, mat4 projectionMatrix, GLuint depthFBO) { //, mat4 view2, mat4 proj2
	...
	glViewport(0, 0, SHADOW_WIDTH, SHADOW_HEIGHT); //ΜΈΓΕΘΟς ΤΟΥ depth texture ΓΙΑ RESOLUTION
	...
	///mat4 view_projection2 = proj2 * view2; //hw2
	//glUniformMatrix4fv(shadowViewProjectionLocation2, 1, GL_FALSE, &view_projection2[0][0]);
...
void lighting_pass(mat4 viewMatrix, mat4 projectionMatrix) {
	...
	*/
	...
	glBindTexture(GL_TEXTURE_2D, depthTexture);
	glUniform1i(depthMapSampler, 2); // Assign sampler to that position
	...
	// Homework 2: send second depth map and light2 VP
	// DISPLAY SHADOWS FROM LIGHT 2
	glActiveTexture(GL_TEXTURE3); // Activate texture position
	glBindTexture(GL_TEXTURE_2D, depthTexture2);
	glUniform1i(depthMapSampler2, 3); // Assign sampler to that position
	// Sending the light2 View-Projection matrix to the shader program
