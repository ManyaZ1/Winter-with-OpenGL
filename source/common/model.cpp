...
void loadOBJWithTiny(
    ...
    for (const auto& shape : shapes) {
        for (const auto& index : shape.mesh.indices) {
            ...
            //The problem was caused by incorrect handling of indices when retrieving normal data from the tinyobj::attrib_t structure.
            ...
            /* “vector subscript out of range” after adding normals*/
            ...
            //The critical step: if (ni + 2 < attrib.normals.size()) checks if the index required for the Z component (which is the highest index) is valid.

            //If the index is out of bounds(which was happening with your Gaea model's normal indices), the program avoids the crash and uses a fallback normal vector (vec3(0.0f, 1.0f, 0.0f)), allowing the model to load and render without crashing.
            vertices.push_back(vertex);
            indices.push_back(indices.size());
        }
    }
